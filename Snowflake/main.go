...
import (
	...
	"github.com/recasta/integrations/redisstream/consumer"
...
func main() {
	...
	_, err = db.Exec(`USE DATABASE recasta;`)
	if err != nil {
		log.Fatalf("failed to use recasta database, err: %v", err)
	}
...
func initDB(db *sql.DB) {
	for _, stmt := range []string{
		`CREATE DATABASE IF NOT EXISTS recasta;`,
